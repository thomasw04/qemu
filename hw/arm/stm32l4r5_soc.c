...
static void stm32l4r5_soc_initfn(Object *obj)
{
    Stm32l4r5SocState *s = STM32L4R5_SOC(obj);
...
{
    ERRP_GUARD();
    Stm32l4r5SocState *s = STM32L4R5_SOC(dev_soc);
    const Stm32l4r5SocClass *sc = STM32L4R5_SOC_GET_CLASS(dev_soc);
    ...
    for (unsigned i = 0; i < NUM_GPIOS; i++) {
        ...
        qdev_prop_set_uint32(dev, "mode-reset",
                             stm32l4r5_gpio_cfg[i].moder_reset);
        qdev_prop_set_uint32(dev, "ospeed-reset",
                             stm32l4r5_gpio_cfg[i].ospeedr_reset);
        qdev_prop_set_uint32(dev, "pupd-reset",
                            stm32l4r5_gpio_cfg[i].pupdr_reset);
        ...
        sysbus_mmio_map(busdev, 0, stm32l4r5_gpio_cfg[i].addr);
    }

    /* System configuration controller */
    busdev = SYS_BUS_DEVICE(&s->syscfg);
    qdev_connect_clock_in(DEVICE(&s->syscfg), "clk", qdev_get_clock_out(DEVICE(&(s->rcc)), "syscfg-out"));
    if (!sysbus_realize(busdev, errp)) {
        return;
    }
...
static void stm32l4r5_soc_class_init(ObjectClass *klass, void *data)
{

    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->realize = stm32l4r5_soc_realize;
    /* Reason: Mapped at fixed location on the system bus */
    dc->user_creatable = false;
    /* No vmstate or reset required: device has no internal state */
}

static void stm32l4r5xi_soc_class_init(ObjectClass *oc, void *data)
{
    Stm32l4r5SocClass *ssc = STM32L4R5_SOC_CLASS(oc);

    ssc->flash_size = 2 * MiB;
}

static void stm32l4r5xg_soc_class_init(ObjectClass *oc, void *data)
{
    Stm32l4r5SocClass *ssc = STM32L4R5_SOC_CLASS(oc);
...
static const TypeInfo stm32l4r5_soc_types[] = {
    {
        .name           = TYPE_STM32L4R5XI_SOC,
        .parent         = TYPE_STM32L4R5_SOC,
        .class_init     = stm32l4r5xi_soc_class_init,
    },{
        .name           = TYPE_STM32L4R5XG_SOC,
        .parent         = TYPE_STM32L4R5_SOC,
        .class_init     = stm32l4r5xg_soc_class_init,
    }, {
        .name           = TYPE_STM32L4R5_SOC,
        .parent         = TYPE_SYS_BUS_DEVICE,
        .instance_size  = sizeof(Stm32l4r5SocState),
        .instance_init  = stm32l4r5_soc_initfn,
        .class_size     = sizeof(Stm32l4r5SocClass),
        .class_init     = stm32l4r5_soc_class_init,
        .abstract       = true,
    }
};

DEFINE_TYPES(stm32l4r5_soc_types)
